...
### 1.12 Seeder isi peran (`role_permissions`) untuk enam role — preset PIC decision-gated
- **Scope:** Seeder tetap mengisi preset `role_permissions` yang **sudah terdefinisi pada baseline resmi** untuk lima role lama: **Superadmin**, **Perencanaan**, **Admin**, **Pimpinan**, dan **Pegawai**. Role keenam **PIC** wajib tersedia sebagai baris `roles` dari 1.6, tetapi **jangan membuat baris `role_permissions` PIC berdasarkan tebakan** selama preset final belum dikonfirmasi. Untuk kebutuhan development sebelum keputusan final, user role PIC boleh menerima grant eksplisit per unit melalui mekanisme 1.14 sehingga resolver, scope, deny, dan workflow dapat diuji tanpa mengubah arti role global. Setelah keputusan preset PIC diterima, sinkronisasi dilakukan melalui task 1.23.
- **Dependency:** 1.10, 1.11, 1.6.
- **DoD:** Test Pest membuktikan: (a) preset `superadmin`, `perencanaan`, `admin`, `pimpinan`, dan `pegawai` cocok persis dengan baseline resmi existing; (b) role `pic` tersedia dan dapat di-assign, tetapi seeder baseline tidak membuat permission PIC yang belum dikonfirmasi; (c) user role PIC dengan grant scoped eksplisit dapat dipakai untuk menguji resolver 1.11 tanpa menjadikan permission scoped sebagai global; (d) menjalankan seeder dua kali tidak menghasilkan duplikasi; (e) test diberi penanda jelas bahwa preset PIC adalah **OPEN**, bukan dianggap kosong sebagai keputusan bisnis final.
...
- **Scope:** Setelah LLDIKTI/Tim Perencanaan memberi keputusan eksplisit mengenai permission bawaan role `pic`, dokumentasikan daftar final tersebut lalu perbarui seeder `role_permissions` 1.12, fixture/test authorization, dan halaman "Jelaskan Izin Pengguna". Task ini **tidak boleh menebak** daftar permission. Bila ada permission `butuh_scope=unit`, keputusan harus menjelaskan apakah permission tetap diperoleh lewat grant unit atau memang dimaksudkan global; jangan memasukkannya ke `role_permissions` secara otomatis.
- **Dependency:** 1.12, 1.22, **keputusan bisnis Q31 preset PIC**.
- **DoD:** Ada referensi keputusan tertulis yang memuat daftar permission PIC; seeder menghasilkan baris PIC yang cocok persis dengan daftar tersebut; test Pest membandingkan preset seeded dengan fixture keputusan; permission unit-scoped tidak menjadi global kecuali keputusan eksplisit menyatakan demikian; seluruh perubahan `role_permissions` teraudit; PRD/Data Model/Workflow/Plan/User Stories/User Issues telah diselaraskan terhadap keputusan yang sama. Task dianggap **BLOCKED/OPEN** sampai keputusan diterima dan tidak boleh ditutup dengan asumsi.
