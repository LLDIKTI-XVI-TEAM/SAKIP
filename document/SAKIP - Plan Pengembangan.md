...
### 5.2 Buat Draft Pengukuran (PIC scoped, Perencanaan global)
...
- **Scope:** Form create/edit/list Kegiatan per periode; permission `kegiatan:create/read/update/delete`; PIC hanya dapat membuat/mengubah kegiatan pada unitnya (scope unit), Perencanaan global.
...
- **Scope:** Pembangunan integrasi saluran pengingat eksternal (WhatsApp Gateway via HTTP API dan Email via Laravel Mail/Notification) yang diselesaikan di bagian akhir sebelum batas waktu 9 November 2026: (1) Notification Channel / Service untuk pengiriman pesan WhatsApp dan Email; (2) Background job broadcast otomatis saat jadwal pengisian triwulan resmi dibuka (`jadwal_periode.pengisian_mulai`) ke seluruh PIC unit kerja terkait; (3) Scheduled command harian (Early Warning System / EWS) yang dijalankan scheduler Laravel setiap hari untuk mengirim pengingat berjenjang (H-7, H-3, H-1 tenggat `pengisian_selesai`) ke WhatsApp dan Email PIC yang indikatornya masih berstatus Draft / Belum Mengisi; (4) Job pengiriman pesan rekapitulasi status pengisian ke WhatsApp/Email Tim Perencanaan (H-3 dan H-1); (5) Job notifikasi instan saat pengembalian berkas pengukuran/rencana aksi beserta catatan revisi wajib ke WhatsApp/Email PIC. Parameter integrasi (saklar aktif, gateway URL, API token, daftar H-minus EWS) dikelola melalui kunci `pengaturan` grup `notifikasi` (Modul 9).
- **Dependency:** 14.1, Modul 3 (jadwal_periode), Modul 9 (pengaturan grup notifikasi), Modul 1.3 (data nomor telepon dan email users).
- **DoD:** Test Pest: simulasi aktivasi jadwal memicu dispatch job broadcast ke antrean (`Queue::fake()`); scheduler EWS harian memfilter daftar PIC dengan pengukuran belum diajukan pada periode aktif dan mendispatch notifikasi WhatsApp/Email sesuai template; aksi pengembalian pengukuran mendispatch notifikasi instan ke antrean; kegagalan respon dari WhatsApp Gateway atau mail server tidak menggagalkan transaksi status pengukuran di database (ditangani secara graceful via try-catch & logged).
...
- **Dependency:** Seluruh migrasi Modul 1–6, Modul 9, Modul 11, Modul 12, Modul 13, Modul 14.1 selesai (2.1–2.20, 3.1–3.10, 4.1, 5.1–5.17, 6.6, 9.1–9.9, 11.1–11.2, 12.1, 12.6, 13.1–13.9, 14.1).
...
|---|---|---|
| 1 | Autentikasi & Akses | Fondasi wajib — seluruh modul lain butuh gate akses & audit dasar; termasuk kelima peran dan katalog permission baru (rencana aksi, kegiatan, komponen, bukti dukung) sejak awal; setup awal memakai Bun sebagai toolchain frontend |
...
| 4 | Penugasan | Butuh Indikator (dari Modul 2); resolusi PIC efektif dipakai ulang oleh Rencana Aksi (Modul 11) |
...
| 14 | Alert Kontekstual & Notifikasi (In-App & Eksternal) | Butuh seluruh modul alur kinerja (Modul 1–13); Task 14.1 (In-App) menjadi baseline penerimaan MVP tertulis (banner dashboard, lonceng navbar, task list), sementara Task 14.2 (WhatsApp & Email Gateway) dikonfigurasi di bagian akhir sebelum batas waktu 9 November 2026 |
| — | Seed Data | Setelah seluruh migrasi Modul 1–6, 9, 11, 12, 13, 14.1 tersedia, sebelum pengujian end-to-end menyeluruh |
