...
class BootstrapSuperadmin
{
    ...
    public function handle(string $subject, string $operator, string $reason, string $runtimeIdentity): bool
    {
        ...
        return DB::transaction(function () use ($subject, $operator, $reason, $runtimeIdentity) {
            ...
            $user = User::where('keycloak_id', $subject)->lockForUpdate()->first();
            if ($marker) {
            ...
            $superadmin = Role::where('kode', 'superadmin')->where('aktif', true)->sole();
            ...
            $expected = ['admin', 'pegawai', 'perencanaan', 'pimpinan', 'superadmin'];
