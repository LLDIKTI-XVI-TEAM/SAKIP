...
class PresentPengukuran
{
    ...
    /**
     * Audit dan target RA dibaca sekaligus per halaman, tanpa query per baris.
     *
    ...
    public function handle(PengukuranKinerja $p, User $actor, bool $detail = false): array
    {
        ...
        $data = ['id' => $p->id, 'versi' => $p->versi, 'status' => $p->status_alur, 'nomor_pengajuan' => $version?->nomor ?? 0, 'jalur_pengajuan' => $version?->jalur_pengajuan,
            ...
            'penugasan_indikator' => ['indikator_kinerja' => $frozen ? $frozen['indikator'] : [...$context->only(['nama', 'satuan', 'arah', 'tipe_perhitungan', 'presisi', 'desimal_tampilan']), 'kode' => $p->indikator->kode, 'definisi_operasional' => $context->definisi],
                'unit_kerja' => $frozen ? $frozen['unit_kerja'] : $context->unit->only(['id', 'nama']), 'pic' => $frozen ? $frozen['pic'] : ($detail ? $p->effectivePic()?->pic?->only(['id', 'nama']) : null)],
            ...
            'bukti_count' => (int) ($p->bukti_dukungs_count ?? 0), 'bukti_dukungs' => [], 'komponen' => [], 'persyaratan_bukti' => [],
        ...
        if ($can['evidence']) {
            $items = $frozen ? $frozen['bukti_dukungs'] : $p->buktiDukungs()->get()->map(fn ($b) => $b->only(['id', 'jenis_berkas_id', 'mode', 'nama_asli', 'mime', 'ukuran_bytes', 'tautan', 'isi_teks']))->all();
