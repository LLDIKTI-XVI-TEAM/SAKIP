...
function DenyLookup<T extends { id: string }>({ id, label, endpoint, value, onChange, describe, disabled, error }: {
    id: string; label: string; endpoint: string; value: string; onChange: (id: string) => void;
    describe: (option: T) => string; disabled: boolean; error?: string;
}) {
    ...
    useEffect(() => {
        ...
        void fetch(`${endpoint}?${new URLSearchParams({ q: query.q, page: String(query.page) })}`, { headers: { Accept: 'application/json' }, signal: controller.signal })
            .then(async (response) => {
                if (!response.ok) throw new Error([401, 403, 419].includes(response.status) ? 'Sesi atau izin berubah. Muat ulang daftar untuk memeriksa akses.' : 'Pilihan belum dapat dimuat. Coba cari kembali.');
    ...
    const applySearch = () => setQuery({ q: search, page: 1 });
    return <div className="space-y-2">
        <div className="flex items-end gap-2">
            <div className="min-w-0 flex-1"><label htmlFor={`${id}-search`} className="block text-sm font-medium">Cari {label.toLowerCase()}</label>
                <input id={`${id}-search`} type="search" maxLength={100} value={search} disabled={disabled} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); applySearch(); } }} className={fieldClass} /></div>
            <Button type="button" variant="outline" className={secondaryButton} disabled={disabled} aria-label={`Cari ${label.toLowerCase()}`} onClick={applySearch}>Cari</Button>
        </div>
        <label htmlFor={id} className="block text-sm font-medium">{label}</label>
        <select id={id} required value={value} disabled={disabled || loading || Boolean(failure)} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : `${id}-status`} className={fieldClass} onChange={(event) => {
        ...
        {(result.page > 1 || result.hasMore) && <div className="flex gap-3">
            <button type="button" disabled={disabled || loading || result.page <= 1} onClick={() => setQuery({ ...query, page: result.page - 1 })} className="rounded p-2 text-sm text-primary underline focus:ring-2 focus:ring-primary disabled:opacity-50">{label} sebelumnya</button>
            <button type="button" disabled={disabled || loading || !result.hasMore} onClick={() => setQuery({ ...query, page: result.page + 1 })} className="rounded p-2 text-sm text-primary underline focus:ring-2 focus:ring-primary disabled:opacity-50">{label} berikutnya</button>
        </div>}
    </div>;
}
...
export function DenyMutationDialog({ deny, actorId, onClose, onSaved }: { deny: DenyRow | null; actorId?: string; onClose: () => void; onSaved: () => void }) {
    ...
    const submit = (event: FormEvent) => {
        ...
        if (form.processing || needsReload || missingSelection) return;
        ...
        form.post(deny ? `/akses/deny/${deny.id}/cabut` : '/akses/deny', {
            ...
            onHttpException: () => { setMessage('Hasil belum dapat dipastikan. Sesi atau izin mungkin berubah. Muat ulang daftar sebelum mencoba kembali.'); return false; },
        });
    };
    return <dialog ref={dialog} aria-labelledby="deny-title" aria-describedby={self ? 'deny-description deny-self-warning' : 'deny-description'} onCancel={(event) => { if (form.processing || message) event.preventDefault(); }} onClose={onClose} className="m-auto max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-xl overflow-y-auto rounded-xl border border-border bg-surface p-5 text-ink shadow-xl backdrop:bg-ink/50 sm:p-6">
        ...
        <form onSubmit={submit} className="mt-5 space-y-5" aria-busy={form.processing}>
            {deny ? <dl className="space-y-2 rounded-lg bg-soft p-4 text-sm">
                ...
                <DenyLookup<UserOption> id="deny-user_id" label="Pengguna" endpoint="/akses/deny/opsi/pengguna" value={form.data.user_id} onChange={(id) => form.setData('user_id', id)} describe={(user) => `${user.nama} (${user.email})${user.is_active ? '' : ' — nonaktif'}`} disabled={form.processing} error={form.errors.user_id} />
                <DenyLookup<PermissionOption> id="deny-permission_id" label="Izin" endpoint="/akses/deny/opsi/izin" value={form.data.permission_id} onChange={(id) => form.setData('permission_id', id)} describe={(permission) => `${permission.kode}${permission.keterangan ? ` — ${permission.keterangan}` : ''}`} disabled={form.processing} error={form.errors.permission_id} />
                ...
                {scope === 'unit' && <DenyLookup<UnitOption> id="deny-unit_id" label="Unit" endpoint="/akses/deny/opsi/unit" value={form.data.unit_id ?? ''} onChange={(id) => form.setData('unit_id', id)} describe={(unit) => `${unit.nama}${unit.status === 'nonaktif' ? ' — nonaktif' : ''}`} disabled={form.processing} error={form.errors.unit_id} />}
            ...
            {needsReload && <div className="space-y-2 rounded-lg bg-soft p-3"><p ref={alert} tabIndex={-1} role="alert" className="text-sm text-danger">{form.errors.deny_id || message}</p><Button type="button" variant="outline" className={secondaryButton} disabled={form.processing} onClick={() => router.get('/akses/deny', {}, { replace: true, preserveState: false })}>Muat ulang daftar</Button></div>}
            <div className="flex flex-wrap justify-end gap-3"><Button type="button" variant="outline" className={secondaryButton} disabled={form.processing || Boolean(message)} onClick={onClose}>Batal</Button><Button type="submit" className={primaryButton} isLoading={form.processing} disabled={needsReload || missingSelection}>{deny ? 'Konfirmasi cabut deny' : 'Simpan deny'}</Button></div>
        </form>
    </dialog>;
}
