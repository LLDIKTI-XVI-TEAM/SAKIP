...
import { LayoutDashboard, FileSpreadsheet, CheckCircle2, LogOut, ChevronRight, CheckCircle, AlertCircle, UserCheck, Menu, X, FileText } from 'lucide-react';
...
export function AuthenticatedLayout({ children, title, breadcrumbs = [] }: AuthenticatedLayoutProps) {
    ...
    const navigation = [
        { href: '/dashboard', label: 'Dashboard Capaian', icon: LayoutDashboard, visible: auth.can.dashboard },
                ...
                <aside id="application-navigation" className={`${navigationOpen ? 'flex' : 'hidden'} flex-col border-b border-border bg-surface md:flex md:w-64 md:shrink-0 md:border-b-0 md:border-r`}>
                    ...
                    <nav aria-label="Navigasi utama" className="flex-1 space-y-1 p-3">
                        {navigation.filter((item) => item.visible).map(({ href, label, icon: Icon }) => {
                            ...
                            return <Link key={href} href={href} aria-current={active ? 'page' : undefined} onClick={() => setNavigationOpen(false)} className={`flex items-center gap-3 rounded-lg px-3 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary ${active ? 'bg-primary text-white' : 'text-ink hover:bg-soft'}`}>
                                <Icon aria-hidden="true" className="h-4 w-4 shrink-0" />{label}
                            </Link>;
                        })}
                    </nav>
                    <div className="border-t border-border p-4">
                        <p className="break-words text-sm font-semibold">{auth.user?.nama}</p>
                ...
                <div className="min-w-0 flex-1">
                    <header className="border-b border-border bg-surface px-4 py-4 sm:px-6">
                        {breadcrumbs.length > 0 && <nav aria-label="Jejak navigasi" className="mb-1 flex flex-wrap items-center gap-1.5 text-xs text-muted">
                            ...
                            {breadcrumbs.map((item, index) => <Fragment key={`${index}-${item.label}`}>
